    ...
    <div class="main-content">
      ...
      <div class="preview-section">
        ...
        <div class="preview-content">
          ...
          <iframe v-else :src="previewUrl" class="preview-iframe" @load="onIframeLoad"></iframe>
        </div>
      </div>
    </div>
...
import {deleteAppOfUserClient, deployAppApi, getAppVOById} from '@/api/AppController.ts'
import type {AppVO} from '@/entity/App.ts'
...
const fetchAppInfo = async () => {
  ...
  try {
    const response = await getAppVOById(id)
    if (response.code === 0 && response.data) {
      appInfo.value = response.data

      // 检查是否有 view = 1 参数，如果有则不自动发送初始提示词
...
const deployApp = async () => {
  ...
  try {
    const response = await deployAppApi({
      appId: appId.value,
    })
    ...
    if (response.code === 0 && response.data) {
      ...
      message.error('部署失败：' + response.message)
    }
  } catch (error) {
...
const deleteApp = async () => {
  ...
  try {
    const response = await deleteAppOfUserClient({ id: appInfo.value.id })
    if (response.code === 0) {
